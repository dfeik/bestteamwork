$(function(){
	...
	//注销
	$("#log_off").click(function () {  //删除的应该是登录的信息，为什么是zhuce
		//删除用户信息cookie
	...
	//购买商品
	$(".context .buy_news>div").eq(2).click(function(){
		if(isLogin()){
...
//判断缓存的数量
function isNum() {
	// var number = document.cookie.indexOf("sum=");
	var number = getCookie("sum");
     console.log(number);
	if(number == 0){
		$("#maincontain").css({
			display:"none",
...
//判断登录状态
function isLogin() {
	var Login = document.cookie.indexOf("username=");
	if(Login == -1){
		$("#on_line").css({
			display:"none",
		});
		...
		$("#on_line #pet_name").text(getCookie("username"));
		return true;
	}
	}
})